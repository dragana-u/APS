package Laboratoriski.edinaesetta.gradovi;
...
class Graph<E> {
    ...
    public float[] dijkstra(int from, String teme) {
        ...
        /* Stringovi za cuvanje vrednostite na teminjата */
        String[] cities = new String[this.num_nodes];
        ...
        finalno[from] = true;
        distance[from] = 0;
        cities[from] = adjList[from].getInfo().toString();
        ...
        for (int i = 1; i < this.num_nodes; i++) {
            ...
            while (it.hasNext()) {
                ...
                if (!finalno[pom.node.getIndex()]) {
                    ...
                    if (distance[pom.node.getIndex()] <= 0) {
                        ...
                        cities[pom.node.getIndex()] = cities[from] + " " + pom.node.getInfo().toString();
                    }
            ...
            for (int j = 0; j < this.num_nodes; j++) {
                if (!finalno[j] && distance[j] != -1) { /* ako cenata ne e konecna */
                    if (!minit) { /* ako ne e inicijализиран minimumот */
                        minc = distance[k = j]; /* proglasi go ova e minimum */
                        minit = true; /* oznaci deka min e inicijализиран */
                    }
        ...
        for (int i = 0; i < cities.length; i++) {
            cities = Arrays.stream(cities).filter(t -> t.contains(teme)).toArray(String[]::new);
            if(cities[i].endsWith(teme)){
...
public class ShortestPathTest {
    public static void main(String[] args) throws IOException {
        ...
        Graph<String> g = new Graph<String>(numCities);
        int numPaths = Integer.parseInt(br.readLine());
        for(int i=0;i<numPaths;i++){
            String[] parts = br.readLine().split("\\s+");
            g.adjList[Integer.parseInt(parts[0])].setInfo(parts[1]);
            g.adjList[Integer.parseInt(parts[2])].setInfo(parts[3]);
            g.addEdge(Integer.parseInt(parts[0]),Integer.parseInt(parts[2]),Float.parseFloat(parts[4]));
        }
        String start = br.readLine();
        String end = br.readLine();
        int ind1=0,ind2=0;
        for(int i=0;i<numCities;i++){
            if(g.adjList[i].getInfo().equals(start)){
                ind1 = g.adjList[i].getIndex();
            }else if(g.adjList[i].getInfo().equals(end)){
                ind2 = g.adjList[i].getIndex();
            }
        }
        float[] dist1 = g.dijkstra(ind1,end);
        float[] dist2 = g.dijkstra(ind2,start);
        System.out.println(dist1[ind2] + dist2[ind1]);
    }
