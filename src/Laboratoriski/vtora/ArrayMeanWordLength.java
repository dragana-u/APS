...
public class ArrayMeanWordLength {
    ...
    public static String wordClosestToAverageLength(Array<String> arr) {
        ...
        for(int j=1;j<arr.getSize();j++){
            if(abs(arr.get(j).length()-average)<abs(max.length()-average)){
                max=arr.get(j);
            }else if(abs(arr.get(j).length()-average)==abs(max.length()-average)){
                if(arr.get(j).length()==max.length() || max.length()>arr.get(j).length()){
                    continue;
                }else{
                    max = arr.get(j);
                }
