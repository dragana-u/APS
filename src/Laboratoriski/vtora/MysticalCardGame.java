...
public class MysticalCardGame {
    ...
    public static void startDuel(SLL<Card> firstSorcererCards, SLL<Card> secondSorcererCards) {
        SLLNode<Card> first1 = firstSorcererCards.getFirst();
        SLLNode<Card> afterFour = secondSorcererCards.getFirst();
        SLLNode<Card> ins = afterFour;
        SLLNode<Card> tmp = first1;
        int max=0,counter=1;
        while (first1 != null) {
            if (first1.element.significance() > max) {
                max = first1.element.significance();
                tmp = first1;
            }
            if(counter==4){
                ins=afterFour;
                ...
                counter++;
                afterFour=afterFour.succ;
            }
            first1=first1.succ;
        }
        firstSorcererCards.delete(tmp);
        secondSorcererCards.insertAfter(tmp.element,ins);
    }
