package ZadaciMinatiGod.Factory;
import java.util.Scanner;
public class Glavna
{
    public static void orders(SLL<Order> aktivna, SLL<Order> ship) {
        SLLNode<Order> activeTmp = aktivna.getFirst();
        int max = activeTmp.element.getPriority();
        SLLNode<Order> maxActive = activeTmp;
        while(activeTmp!=null){
            if(activeTmp.element.getPriority()<=max){
                max = activeTmp.element.getPriority();
                maxActive = activeTmp;
            }
            activeTmp=activeTmp.succ;
        }
        aktivna.delete(maxActive);
        SLLNode<Order> shipTmp = ship.getFirst();
        ship.insertLast(maxActive.element);
    }
