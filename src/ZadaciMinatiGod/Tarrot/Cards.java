package ZadaciMinatiGod.Tarrot;
...
public class Cards
{
    public static void tarotCards(SLL<Card> firstPart,SLL<Card> secondPart) {
        secondPart.insertLast(firstPart.getFirst().element);
        firstPart.deleteFirst();
        firstPart.insertLast(secondPart.getFirst().element);
        secondPart.deleteFirst();

        SLLNode<Card> tempFirst = firstPart.getFirst();
        ...
        SLLNode<Card> tmpSecond = secondPart.getFirst();
        count=1;
        double size2 = Math.ceil(secondPart.length()/2.0);
        while(true){
            if(count==size2){
                secondPart.insertAfter(tempFirst.element,tmpSecond);
                break;
            }
            count++;
            tmpSecond=tmpSecond.succ;
        }
        firstPart.delete(tempFirst);
    }
