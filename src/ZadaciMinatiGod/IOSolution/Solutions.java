package ZadaciMinatiGod.IOSolution;
...
public class Solutions
{
    public static void alterTeams(SLL devTeam, SLL qaTeam) {
        SLLNode tmpQA = qaTeam.getFirst();
        SLLNode najmlad = tmpQA;
        int min = najmlad.age;
        while(tmpQA!=null){
            if(tmpQA.age<=min){
                min = tmpQA.age;
        ...
        double len = Math.ceil(devTeam.length()/2.0);
        int brojac = 1;
        SLLNode tmpDev = devTeam.getFirst();
        SLLNode sredina = tmpDev;
        while(tmpDev!=null){
            if(brojac == len){
                devTeam.insertAfter(najmlad.id, najmlad.age, tmpDev);
                break;
            }
            tmpDev = tmpDev.succ;
            brojac++;
